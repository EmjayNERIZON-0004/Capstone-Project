...
<body>
    ...
    <div>
    <label class="m-0 d-block">What service did you avail in the {{$selected_sub_office['sub_office_name']}} Office?<span class="text-danger">*</span></label>
<small class="text-muted d-block">Anong partikular na serbisyo ang iyong natanggap mula sa opisina?</small>
  </div> </div>
</div>

 <form action="{{ route('submit-services') }}" method="POST">
    @csrf
    ...
    <!-- <input type="text" name=" " value="{{$selected_sub_office['id']}}"> -->

    <!-- <label for="sub_office">Select Sub Office:</label> -->
    <select class="form-select mt-2"  name="services_id"  style="border-bottom: 2px solid rgb(4, 59, 125);"  required >
        @foreach ($services as $servicesAvailed)
            <option value="{{ $servicesAvailed['id'] }}" data-main="{{ $servicesAvailed['service_name'] }}">
                {{ $servicesAvailed['service_name'] }}
            </option>
        @endforeach

        <option value="others">Other Request/Inquiries</option>
    </select>
...
<div class="progress-container">
    <span class="progress-text">Page 4 of 6</span>
    <div class="progress">
        <div class="progress-bar" style="width: 55%;"></div> <!-- Adjust width dynamically -->
    </div>
</div>
</div>
