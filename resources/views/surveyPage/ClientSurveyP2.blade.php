...
<head>
    ...
    <title>Schools Division Office Form</title>
...
<div class="container mt-4">
    ...
    <div>
        <label class="m-0 d-block">Which office did you transact with?<span class="text-danger">*</span></label>
        <small class="text-muted d-block">Aling opisina ang iyong pinuntahan para sa iyong transaksyon?</small>
    </div>
</div>

    </div>
       
                    <form action="{{ route('check.office') }}" method="POST">
...
@php
    $mainOfficeFromUrl = request()->query('mainOffice');
@endphp

<select class="form-select" name="main_office" id="main_office" style="border-bottom: 2px solid rgb(4, 59, 125); color: black;">
    @foreach ($mainOffices as $office)
        <option 
            value="{{ $office['office_id'] }}"
            {{ ($mainOfficeFromUrl == $office['office_id']) ? 'selected' : '' }}
            {{ ($mainOfficeFromUrl && $mainOfficeFromUrl != $office['office_id']) ? 'disabled' : '' }}
        >
            {{ $office['office_name'] }}
        </option>
    @endforeach
...
<div class="progress-container">
    <span class="progress-text">Page 2 of 6</span>
    <div class="progress">
        <div class="progress-bar" style="width: 33%;"></div> <!-- Adjust width dynamically -->
    </div>
</div>
</div>
